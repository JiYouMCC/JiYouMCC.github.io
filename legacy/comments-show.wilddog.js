...
function getCommentByCommentId(commentId) {
  for (var i = 0; i < comments_messages.length; i++) {
    var comment = comments_messages[i];
...
function showComments(page) {
  ...
  Comments.comment.listCallback(page, function(comments) {    
    ...
    for (var i = 0; i < comments.length; i++) {
      ...
      if (comment.reply) {
        var reply_comment = getCommentByCommentId(comment.reply);
        comment_detail.append(
          $("<div></div>").append(
            ...
            $("<div></div>").addClass('page_blog_comment_message_reply').text(reply_comment.comment)
          )
        )
      }
    ...
    $("button[id^=button_reply_]").click(function() {
      ...
      $("#reply").val(reply);
      $("#reply_user").text(getCommentByCommentId(reply).name);
      $("#reply_to").show();
      $("html,body").animate({
