...
1. 首先，老牌[xlrd](https://xlrd.readthedocs.io/en/latest/){:target="_blank"}是不能用的，因为它不支持xlsx:facepalm:，虽然我平时都用它
2. 然后，[xlwings](https://www.xlwings.org/)是可以用的，但是它本质还是会起一个excel的app进行操作，虽然可以让它不显示窗口，但是实际应用的时候发现删除shape是一个一个删除的，于是就会有这样的画风:face_with_spiral_eyes:：
```
    python excel.py
    里面有53074个图形
...
```
    ...
    app.quit()
```
3. 最后我突然灵光一闪。主要还是在搜怎么删除图形又不用打开后台。我一直怀疑xlwings是删一个存一个或者删一个渲染一个才会那么那么慢，发现有人在steakoverflow上说，openpyxl不支持图形和图片，如果打开已有的会ignore！于是出现了以下代码：
```
  import openpyxl
  import xlwings
